        ...
        el('div', { class: 'list-item' }, [
          el('div', {}, [
            ...
            el(
              ...
              { class: 'meta' },
              [
                ...(!excludedKinds.has(SEGMENT_KINDS.UP) ? [`up ${formatDuration(c.segments[SEGMENT_KINDS.UP]?.durationMs ?? 0)}`] : []),
                ...(!excludedKinds.has(SEGMENT_KINDS.TOP_REST) ? [`top ${formatDuration(c.segments[SEGMENT_KINDS.TOP_REST]?.durationMs ?? 0)}`] : []),
                ...(!excludedKinds.has(SEGMENT_KINDS.DOWN) ? [`down ${formatDuration(c.segments[SEGMENT_KINDS.DOWN]?.durationMs ?? 0)}`] : []),
                ...(!excludedKinds.has(SEGMENT_KINDS.BOTTOM_REST) ? [`bot ${formatDuration(c.segments[SEGMENT_KINDS.BOTTOM_REST]?.durationMs ?? 0)}`] : []),
              ].join(' · '),
            ),
          ]),
